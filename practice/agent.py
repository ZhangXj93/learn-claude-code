    ...
    while True:
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                print_labeled("需要执行的命令", block.input["command"])
...
if __name__ == "__main__":
    ...
    print_labeled("使用说明", "输入问题，回车发送。输入 q 退出。")
    ...
    while True:
        ...
        response_blocks = cast(list[object], response_content) if isinstance(response_content, list) else []
        for block in response_blocks:
            if getattr(block, "type", None) == "text":
                print_labeled("最终回复", str(getattr(block, "text", "")))
        print()
